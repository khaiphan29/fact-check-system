...
const NumberLineChart = ({ selectedYear, selectedMonth }: { selectedYear: SelectedYear, selectedMonth: SelectedMonth }) => {
  ...
  if(value == filledFactCheckData?.year && value2 == 13){
    ...
    if(filledFactCheckData){
      ...
      ApprovedList = sortedByDay.map((dayData) => dayData.statistic.total_approved)
      RefusedList = sortedByDay.map((dayData) => dayData.statistic.total_refuted)
      NeutralList = sortedByDay.map((dayData) => dayData.statistic.total_neutral)
      TotalList = sortedByDay.map((dayData) => dayData.statistic.total_fact_check)
      AvarageList = sortedByDay.map((dayData) => Math.floor(dayData.statistic.total_fact_check/3))
    }
