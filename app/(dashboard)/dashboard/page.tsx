...
const Dashboard = () => {
  const [selectedYear, setSelectedYear] = useState<{ value: number; label: string }>({ value: 2023, label: "2023" });
  const [selectedMonth, setSelectedMonth] = useState<{ value2: number; label: string }>({ value2: 13, label: 'Toàn bộ' });
  ...
  const yearOptions = [
    ...
    return (
        ...
        <div className="mt-12 mb-8 grid-cols-6 grid gap-4 md:mt-6 md:gap-6 2xl:mt-7 2xl:gap-7">
            <NumberLineChart selectedYear={selectedYear} selectedMonth={selectedMonth}/>
        </div>
        
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-5 2xl:gap-7">
            <Card
            data={Data?.statistic.total_fact_check ? Data?.statistic.total_fact_check : 0}
            label={`Tổng Tin Được Kiểm Tra Trong ${Info}`}
            icon={<CgEye size={25} />}
            />

            <Card
            data={Data?.statistic.total_refuted ? Data?.statistic.total_refuted : 0}
            label={`Tổng Tin Giả Được Phát Hiện Trong ${Info}`}
            icon={<CgCloseO size={25} />}
            />
            
            <Card
            data={Data?.statistic.total_approved ? Data?.statistic.total_approved : 0}
            label={`Tổng Tin Được Xác Thực Trong ${Info}`} //Most Checked Topic This Month
            icon={<BsFillPersonCheckFill size={25} />}
            />

            <Card
            data={Data?.statistic.total_neutral ? Data?.statistic.total_neutral : 0}
            label={`Tổng Tin Không Xác Định Trong ${Info}`}   //Most Faked Topic This Month
            icon={<FiAlertOctagon size={25} />}
            />

            <Card
            data={Data?.popular_source ? Data?.popular_source : "không có dữ liệu"}
